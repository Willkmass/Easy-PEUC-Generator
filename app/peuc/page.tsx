          ...
          {peucs.map((peuc) => (
            <div key={peuc.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition flex flex-col justify-between">
              ...
              <div className="flex gap-2 pt-3 border-t border-slate-100">
                <button
                  onClick={() => alert(`Dados brutos da PEUC:\n\n${JSON.stringify(peuc, null, 2)}`)}
                  className="w-full text-center bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold py-2 rounded-md"
                >
                  Ver Detalhes
                </button>
                <button
                  onClick={() => excluirPEUC(peuc.id)}
                  className="bg-red-50 hover:bg-red-100 text-red-600 text-xs font-semibold px-3 py-2 rounded-md"
                >
                  Excluir
                </button>
              </div>
